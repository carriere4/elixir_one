defmodule Identicon do
  ...
  def draw_image(%Identicon.Image{color: color, pixel_map: pixel_map}) do #we don't need image struct, so we don't need that as an argument
    ...
    Enum.each pixel_map, fn({start, stop}) ->
      :egd.filledRectangle(image, start, stop, fill) #an oddity of egd - we are actually transforming the image variable
    end
  ...
  def filter_odd_squares(%Identicon.Image{grid: grid} = image) do
    ...
    %Identicon.Image{image | grid: grid} # creates new image with new grid
  end
  ...
  def build_grid(%Identicon.Image{hex: hex} = image) do
    ...
    %Identicon.Image{image | grid: grid}  #creates new image by adding grid
  end
